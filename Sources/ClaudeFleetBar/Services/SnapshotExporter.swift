...
enum SnapshotExporter {
    ...
    static func write(_ usages: [AccountUsage]) {
        let ordered = Ranking.runOrder(usages)
        let payload: [String: Any] = [
            "generated_at": ISO8601.string(.now),
            "recommended": Ranking.recommended(usages)?.account.label as Any? ?? NSNull(),
            ...
            "accounts": ordered.map { usage in
                ...
                row["headroom"] = Ranking.headroom(usage) as Any? ?? NSNull()
